...
import { makeLayoutAnimation } from "../helpers/utils";
export default class Wizard extends PureComponent {
  ...
  handleScroll = (e) => {
    ...
    if (
      e.nativeEvent.contentOffset.x < width * step + 1 ||
      e.nativeEvent.contentOffset.x > width * step - 1
    ) {
  ...
  renderStep = (item, index) => {
    ...
    const handleRenderAlways =
      item.renderAlways === true ||
      (item.renderAlways === false && index === step);

    if (
      handleRenderAlways ||
      handleDependsOn ||
      (item.renderAlways === undefined && item.depensOn === undefined)
    ) {
      if (index === step) {
        return (
      ...
      return (
        <View
          key={index}
          style={[allowChangeHeight && { height: 0 }, { width }]}
        >
          {item.content}
        </View>
      );
  ...
  componentDidMount() {
    ...
    step !== 0 &&
      this.scrollView.scrollTo({ x: step * width, y: 0, animated: false });
